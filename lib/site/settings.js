...
import { safeHttpUrl, safeImageSrc, safeMapEmbed, safeTheme } from "./sanitize";

export { safeHttpUrl, safeImageSrc, safeMapEmbed, safeTheme };
...
export function missingFacts(settings) {
	...
	if (settings.results.some((result) => result.sample)) gaps.push("Real member results");
	return gaps;
}
