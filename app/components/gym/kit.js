...
function buildDoor(THREE, mat) {
	...
	group.add(slab(THREE, 1.9, 0.03, 1.2, mat.rubber, x - 1.6, 0.02, 0.55));
...
function buildIron(THREE, mat) {
	...
	group.add(slab(THREE, 1.5, 0.11, 0.42, mat.pad, x + 3.4, 0.48, -2.1));
	group.add(slab(THREE, 0.14, 0.36, 0.14, mat.steel, x + 3.4, 0.24, -2.1));
	for (const dx of [-0.62, 0.62]) group.add(slab(THREE, 0.12, 0.1, 0.72, mat.steel, x + 3.4 + dx, 0.06, -2.1));
...
function buildClasses(THREE, mat) {
	...
	for (let i = 0; i < 4; i += 1) {
		group.add(slab(THREE, 1.28, 0.04, 1.9, i % 2 ? mat.pad : mat.rubber, x - 2 + i * 1.32, 0.02, 0.3));
	}
...
function buildDesk(THREE, mat, plans) {
	...
	group.add(slab(THREE, 3.3, 1.02, 0.72, mat.dark, x + 0.2, 0.51, -1.25));
	group.add(slab(THREE, 3.5, 0.08, 0.88, mat.steel, x + 0.2, 1.06, -1.22));
	group.add(slab(THREE, 3.3, 0.06, 0.06, mat.paint, x + 0.2, 0.72, -0.88));
	...
	group.add(slab(THREE, 4.3, 0.07, 0.07, mat.paint, railX + 1.85, 0.52, railZ));
	for (const dx of [-0.1, 3.8]) group.add(slab(THREE, 0.09, 0.55, 0.09, mat.dark, railX + dx, 0.27, railZ));
...
export function buildKit(THREE, { mat, plans }) {
	...
	group.add(door.group, iron.group, classes.group, recovery.group, desk.group);
	...
	return { group, apply, plates: desk.plates, bag: classes.bag, bar: iron.bar };
}
